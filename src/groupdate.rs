...
const SQLITE_DATA_TMP_PATH: &'static str = "/tmp";
...
impl GroupDate {
    ...
    pub fn construct_sqlite3_temporary_filename(&self) -> Result<String,Error> {
        ...
        Ok(format!("{}/{}.{}.sqlite3",SQLITE_DATA_TMP_PATH,&self.ym,self.group.id()))
    }
    ...
    async fn group_status_id(&self, conn: &mut Connection) -> Result<usize,Error> {
        ...
        let sql = format!("SELECT `id` FROM `group_status` WHERE `group_id`=? AND `year`=? AND `month`=?");
        let group_status_id: usize = conn.prepare(&sql)?
            .query_map((group_id,year,month,), |row| Ok(row.get(0)?))?
            .next()
    ...
    async fn get_globalimagelinks(&self, files: &[String], baglama: &Baglama2) -> Result<Vec<GlobalImageLinks>,Error> {
        if files.is_empty() {
            return Ok(vec![]);
        }
    ...
    async fn add_views_for_files_to_sqlite(&self, conn: &mut Connection, all_files: &Vec<String>, baglama: &Baglama2) -> Result<(),Error> {
        ...
        for files in all_files.chunks(CHUNK_SIZE) {
            ...
            for gil in &globalimagelinks {
                ...
                let sql_value = format!("({site_id},?,{month},{year},{done},{namespace_id},{page_id},{views})");
            ...
            if !parts.is_empty() {
                ...
                let placeholders: Vec<String> = parts.iter()
                    .map(|part|format!("(`site`={} AND `title`=?)",part.0))
                    .collect();
                let sql = "SELECT id,site,title FROM `views` WHERE ".to_string() + &placeholders.join(" OR ").to_string();
                // println!("{sql}\n{site_titles:?}");
                // conn.execute(&sql, params_from_iter(site_titles))?;
                let viewid_site_id_title: Vec<(usize,usize,String)> = conn.prepare(&sql)?
                    .query_map(params_from_iter(site_titles), |row| Ok((row.get(0)?,row.get(1)?,row.get(2)?)))?
                    .filter_map(|x|x.ok())
                    .collect();
                let siteid_title_viewid: HashMap<(usize,String),usize> = viewid_site_id_title.into_iter()
                ...
                if !values.is_empty() {
                    let sql = "INSERT OR IGNORE INTO `group2view` (group_status_id,view_id,image) VALUES ".to_string() + &values.join(",").to_string();
                    // println!("{sql}\n{images:?}\n");
    ...
    async fn get_files_from_commons_category_tree(&self, category: &str, depth: usize, baglama: &Baglama2) -> Result<Vec<String>,Error> {
        baglama.get_pages_in_category(category,depth,6).await
    }

    pub async fn seed_sqlite_file(&self, conn: &mut Connection, baglama: &Baglama2) -> Result<(),Error> {
        ...
        for site in &sites {
            ...
            conn.execute(&sql, rusqlite::params![site.id(),site.grok_code,site.server,site.giu_code,site.project(),site.language(),site.name])?;
        }
        ...
        if let Some(group) = groups {
            let sql = "INSERT INTO `groups` (id,category,depth,added_by,just_added) VALUES (?,?,?,?,?)" ;
            conn.execute(&sql, rusqlite::params![group.id,group.category,group.depth,group.added_by,group.just_added])?;
        }
        ...
        if let Some(gs) = group_status {
            ...
            conn.execute(&sql, rusqlite::params![gs.id,gs.group_id,gs.year,gs.month,gs.status,gs.total_views,gs.file,gs.sqlite3])?;    
        } else {
        ...
        conn.execute("UPDATE `group_status` SET `status`='',`total_views`=null,`file`=null,`sqlite3`=null", ())?;
        Ok(())
    }

    pub async fn add_files_to_sqlite(&self, conn: &mut Connection, baglama: &Baglama2) -> Result<(),Error> {
        let conn = conn;
        let sql = "SELECT `category`,`depth` FROM `groups` WHERE `id`=?";
        let (category, depth): (String,usize) = conn.prepare(sql)?
        ...
        let files = self.get_files_from_commons_category_tree(&category, depth, baglama).await?;
        ...
        for file in &files {
            ...
            if parts.len()>=batch_size {
                ...
                let sql = format!("INSERT INTO `files` (`filename`) VALUES {}",questionmarks.join(","));
    ...
    pub async fn add_pages_to_sqlite(&mut self, conn: &mut Connection, baglama: &Baglama2) -> Result<(),Error> {
        ...
        loop {
            ...
            let files: Vec<String> = conn.prepare(&sql)?
                .query_map([], |row| row.get(0))?
                .filter_map(|x|x.ok()) // TODO something more elegant?
                .collect();
            let _ = self.add_views_for_files_to_sqlite(conn, &files, baglama).await;
            offset += files.len();
    ...
    // TESTED
    pub async fn get_total_monthly_page_views(&self, server: &str, title: &str, first_day: &str, last_day: &str) -> Option<u64> {
    ...
    async fn view_done(&self, view_id: usize, done: u8, conn: &mut Connection) -> Result<usize,Error> {
        conn.execute("UPDATE `views` SET `done`=?1,`views`=0 WHERE `id`=?2", rusqlite::params![done,view_id])
        .map_err(|e|Error::from(e))
    }

    async fn update_view_count(&self, view_id: usize, view_count: u64, conn: &mut Connection) -> Result<usize,Error> {
        conn.execute("UPDATE `views` SET `done`=1,`views`=?1 WHERE `id`=?2", rusqlite::params![view_count,view_id])
        .map_err(|e|Error::from(e))
    }

    async fn get_view_counts(&self,conn: &mut Connection, batch_size: usize) -> Result<Vec<ViewCount>,Error> {
        ...
        let rows = stmt.query_map([], |row| ViewCount::from_row(row))?;
        let ret: Vec<ViewCount> = rows.into_iter().filter_map(|row|row.ok()).collect();
        Ok(ret)
    }

    pub async fn add_view_counts_to_sqlite(&mut self, conn: &mut Connection, baglama: &Baglama2) -> Result<(),Error> {
        ...
        while found {
            ...
            for vc in rows {
                ...
                match baglama.prefix_with_namespace(&vc.title, vc.namespace_id, &wiki).await {
                    Some(title) => {
                        views_todo.push((server.to_string(),title.to_string(),first_day.to_string(),last_day.to_string(),vc.view_id));
            ...
            if !views_todo.is_empty() {
                ...
                for views_todo_batch in views_todo.chunks(10) {
                    let futures: Vec<_> = views_todo_batch.iter()
                        .map(|(server,title,first_day,last_day, _view_id)|self.get_total_monthly_page_views(&server, &title, &first_day, &last_day))
                        .collect();
                    let results: Vec<u64> = join_all(futures).await.iter().map(|r|r.unwrap_or(0)).collect();
                    // println!("Futures complete");
                    for (view_count,(_server,_title,_first_day,_last_day, view_id)) in results.into_iter().zip(views_todo_batch.iter()) {
    ...
    async fn finalize_sqlite(&self, conn: &mut Connection, sqlite_filename: &str, baglama: &Baglama2) -> Result<(),Error> {
        let conn = conn;
        ...
        let sql = format!("SELECT ifnull(total_views,0) FROM group_status WHERE id=?");
        let total_views: usize = conn.prepare(&sql)?
            .query_map([group_status_id], |row| row.get(0))?
            .next()
            .unwrap_or(Ok(0))?;

        conn.execute("CREATE INDEX `views_views_site_done` ON `views` (`site`,`done`,`views`)",())?;
        conn.execute("CREATE INDEX `g2v_view_id` ON `group2view` (`view_id`)",())?;
        self.set_group_status("VIEW DATA COMPLETE",total_views,sqlite_filename,baglama).await?;
        Ok(())
    }

    pub async fn set_group_status(&self, status: &str, total_views: usize, sqlite_filename: &str, baglama: &Baglama2) -> Result<(),Error> {
        if let Ok(mut conn) = baglama.get_tooldb_conn().await {
            ...
            let _ = conn.exec_drop(sql, mysql_async::params! {group_id,year,month,status,total_views,sqlite_filename}).await;
        }
        Ok(())
    }
    ...
    pub async fn create_sqlite(&mut self, baglama: &Baglama2) -> Result<(),Error> {
        ...
        println!("{}: {} [ {fn_work} => {fn_final} ]",&self.ym,self.group.id());
...
mod tests {
    ...
    async fn test_get_total_monthly_page_views() {
        ...
        let total = gd.get_total_monthly_page_views("en.wikipedia.org", "Eliza_Maria_Gordon-Cumming", "20221001", "20221031").await;
