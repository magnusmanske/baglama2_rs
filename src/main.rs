...
fn month(month: Option<&String>) -> u32 {
    match month.map(|s|s.as_str()) {
        ...
        Some(s) => {
            s.parse::<u32>().expect("Month: number expected, not {s}")
        }
...
fn year(year: Option<&String>) -> i32 {
    match year.map(|s|s.as_str()) {
        ...
        Some(s) => {
            s.parse::<i32>().expect("Year: number expected, not {s}")
        }
...
async fn main() -> Result<(),Error> {
    ...
    match argv.get(1).map(|s|s.as_str()) {
        Some("run") => {
            let group_id = argv.get(2).map(|s|s.parse::<usize>().unwrap()).expect("Group ID expected");
            let year = year(argv.get(3));
            let month = month(argv.get(4));
            let mut gd = GroupDate::new(group_id,YearMonth::new(year,month));
            let _ = gd.set_group_status("GENERATING PAGE LIST", 0, "", &baglama).await;
        ...
        Some("next") => {
            ...
            if let Some(group_id) = baglama.get_next_group_id(year,month).await {
                GroupDate::new(group_id,YearMonth::new(year,month)).create_sqlite(&baglama).await?;
            } else {
        ...
        Some("next_all_seq") => {
            let year = year(argv.get(2));
            let month = month(argv.get(3));
            if argv.get(4).is_none() {// Any third parameter will do this
